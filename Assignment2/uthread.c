...
static void run_thread_func(void (*func)(void *), void* args) {
  alarm(THREAD_QUANTA);
  func(args);
  uthread_exit();
}

int  uthread_create(void (*func)(void *), void* args)
{
  int i;
...
found:
  ...
  *((int*)(threads[i].stack + STACK_SIZE - 1*sizeof(int))) = (int)args;
...
void uthread_schedule(void)
{
  ...
  if(threads[current_thread_index].esp == 0) {
    ...
    // current call to uthread_schedule returns, execution will return to where
